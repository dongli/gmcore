...
contains
  ...
  subroutine history_init()
    ...
    call io_add_var('h1', 'dkedlon'  , long_name='zonal KE gradient force'           , units=''       , dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('h1', 'dkedlat'  , long_name='meridional KE gradient force'      , units=''       , dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('h1', 'dpedlon'  , long_name='zonal PE gradient force'           , units=''       , dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('h1', 'dpedlat'  , long_name='meridional PE gradient force'      , units=''       , dim_names=['lon ', 'ilat', 'time'])
    ...
    call io_add_var('h1', 'dpv_lon_t', long_name='meridional dpv on U grid'          , units='m-1 s-1', dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('h1', 'dpv_lat_n', long_name='meridional dpv on V grid'          , units='m-1 s-1', dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('h1', 'dpv_lat_t', long_name='zonal dpv on V grid'               , units='m-1 s-1', dim_names=['ilon', 'lat ', 'time'])
  ...
  subroutine history_write_debug(static, state, tend)
    ...
    call io_output('h1', 'dkedlat'  , tend%dkedlat     (1:mesh%num_full_lon,1:mesh%num_half_lat))
    call io_output('h1', 'dpedlon'  , tend%dpedlon     (1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('h1', 'dpedlat'  , tend%dpedlat     (1:mesh%num_full_lon,1:mesh%num_half_lat))
